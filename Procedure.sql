...
BEGIN
    ...
    BEGIN
		...
		ELSE 
		BEGIN
			IF @MALOAIKHUYENMAI IN ('1') 
			BEGIN
				IF EXISTS (SELECT 1 FROM SANPHAM_KHUYENMAI WHERE KHUYENMAIID = @MAKHUYENMAI AND MASP = @MASP1)
				BEGIN
			...
			IF @MALOAIKHUYENMAI IN ('3') 

			BEGIN
				IF EXISTS (SELECT 1 FROM KHUYENMAI_KHACHHANG WHERE MAKHUYENMAI = @MAKHUYENMAI AND MUCKHTT = @MUCKHTT)
					BEGIN
						THROW 50005, N'MỨC KHÁCH HÀNG ĐÃ ÁP DỤNG KHUYẾN MÃI', 1;
					END	
				IF EXISTS (SELECT 1 FROM SANPHAM_KHUYENMAI SPKM JOIN KHUYENMAI_KHACHHANG KMKH ON SPKM.KHUYENMAIID = KMKH.MAKHUYENMAI
...
EXEC SP_THEM_KHUYEN_MAI '16', 3, NULL, '1', 50,'2024-12-28','2024-12-30', 10, NULL
EXEC SP_THEM_KHUYEN_MAI '17', 1, 2, '2', 50,'2024-12-28','2024-12-30', 10, NULL
EXEC SP_THEM_KHUYEN_MAI '19', 2, NULL, '3', 50,'2024-12-28','2024-12-30', 20, N'BẠC'
*/
...
BEGIN
	...
	IF EXISTS(SELECT 1 FROM SANPHAM AS SP WITH(REPEATABLEREAD) WHERE SP.MASP=@MASP)
		BEGIN
			IF(@SOLUONG>0)
				BEGIN
					INSERT INTO CHITIETDONHANG WITH (XLOCK) (DONHANGID, MASP, SOLUONG) VALUES (@DONHANGID, @MASP, @SOLUONG)
					COMMIT;
				END
			ELSE
				BEGIN
